...
class GHLCalendarAPI:
    ...
    def get_date_time(self , file_path):
        with open(file_path, "r") as json_file:    
            data_dict = json.load(json_file)

        print("Data Dict:", data_dict)
        data_dict_clean = {key.lstrip('- '): value if '-' in key else value for key, value in data_dict.items()}
        date_selected = data_dict_clean["dateSelected"]
        time_selected = data_dict_clean["timeSelected"]
    ...
    def get_calender(self , location_id , api_key):
        calendar_id = None
    ...
    def fetch_available_slots(self , calender_id , api_key , start_date, end_date, time_24h_format, date_selected):
        try:
            ...
            if time_check_str in nearest_slots_str:
                ...
                return "No time slot is available" , nearest_slots_str , "Time SLot is not Available"
